...
const IconGlyph = ({ type }) => {
  if (type === "loop") {
    return (
      <svg viewBox="0 0 120 120" aria-hidden="true" focusable="false">
        <g clipPath="url(#loopClip)">
  ...
  if (type === "blog") {
    return (
      <svg viewBox="0 0 120 120" aria-hidden="true" focusable="false">
        <line x1="22" y1="36" x2="98" y2="36" stroke="currentColor" strokeWidth="9" strokeLinecap="round" />
        <line x1="22" y1="60" x2="86" y2="60" stroke="currentColor" strokeWidth="9" strokeLinecap="round" />
        <line x1="22" y1="84" x2="74" y2="84" stroke="currentColor" strokeWidth="9" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  useEffect(() => {
    const updateHighlight = () => {
      ...
      if (mx !== Infinity && el) {
        ...
        const nx = (mx - cx) / (rect.width * 1.2);
        const ny = (my - cy) / (rect.height * 1.2);
        lightX = 50 + nx * 30;
        lightY = 50 + ny * 30;
      } else if (tilt?.active) {
        lightX = 50 + tilt.x * 20;
      ...
      surface.style.setProperty("--highlight-x", `${clamp(lightX, 8, 92)}%`);
      surface.style.setProperty("--highlight-y", `${clamp(lightY, 8, 92)}%`);
    };

    const unsub = mouseClientX.on("change", updateHighlight);
    updateHighlight();
    return () => unsub();
  }, [mouseClientX, mouseClientY, tilt]);
  ...
  return (
    ...
    >
      <MotionDiv
        ...
        ref={surfaceRef}
      >
        <span className="dock-icon__glyph">
          <IconGlyph type={type} />
        </span>
      </MotionDiv>
